...
import { fetch, update, deleteItem, setRefetch, deleteItemsByIds } from '../../stores/students/studentsSlice'
...
const TableSampleStudents = ({ filterItems, setFilterItems, filters, showGrid }) => {
  ...
  const { students, loading, count, notify: studentsNotify, refetch } = useAppSelector((state) => state.students)
  ...
  useEffect(() => {
    loadColumns(handleDeleteModalAction, `students`).then((newCols) =>
      setColumns(newCols),
    );
  }, []);
